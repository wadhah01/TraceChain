...
const ContractAddress = "0xe3CE9f480AEAe1109FA66d30D34B96F51731670C";
...
export const TrackingProvider = ({ children }) => {
    ...
    const getShipmentsByProductId = async (productId) => {
        try {
          const provider = new ethers.providers.JsonRpcBatchProvider();
          const contract = fetchContract(provider);
          const sender = currentUser; // Assuming currentUser holds the sender's address
          const shipments = await contract.getShipmentsByProductId(sender, productId);
          // Process the shipments data if needed
          return shipments;
        } catch (error) {
          console.log("Error fetching shipments by product ID", error);
        }
      };
