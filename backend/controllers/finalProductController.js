...
const deleteFPrById = async(req,res)=>{
    ...
    try{    
        ...
        res.status(201).send(findFPByCreator(),FPDeleted);
    }catch(err){
...
const findFPById = async(req,res)=>{
...
const findFPByCreator = async(req,res)=>{
    
    try {
        const creatorID = req.params.creator;
        const FP = await FinalProduct.find({ createdBy: creatorID });
        if (FP) {
...
const updateFPById = async (req, res) => {
    try {
        ...
        const FPId = req.params.id;   
        const {name, type, stock, price, dateProduction, delay, lotNumber, ingredient, createdBy, FPImage } = req.body;
